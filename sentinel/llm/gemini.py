...
class GeminiScout(LLMProvider[str]):
    ...
    def __init__(self, model_name: str = "gemini-1.5-flash", client: httpx.Client | None = None) -> None:
    ...
    def generate(self, prompt: str) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Gemini Scout request failed: {exc}") from exc
        if not isinstance(text, str) or not text.strip():
            raise RuntimeError("Gemini Scout returned no text output")
        return text
